...
public abstract class BaseSigner {
    ...
    public byte[] sign(InputStream is, RubricaSigner signer, Certificate[] certChain, Properties params)
        ...
        try (PdfReader pdfReader = new PdfReader(is) {
            ...
            if (signTime == null || signTime.isBlank()) {
                signTime = java.time.OffsetDateTime.now().format(java.time.format.DateTimeFormatter.ISO_OFFSET_DATE_TIME);
                extraParams.setProperty(SIGN_TIME, signTime);
            }
            // Tamaño letra
            float fontSize = 3;
            try {
